...
function sv_events:hasEvent(event) return self._Events[event] end
...
function sv_events:unobserve(event, callback)
    if self:hasEvent(event) then
        ...
        for _, cb in self._Callbacks[event] do if not callback == cb then table.insert(tmp, cb) end end
...
function sv_events:emit(event, data, ...)
    ...
    if self:hasEvent(event) and (not self:isSilenced(event)) then
        ...
        for idx, callback in ipairs(self._Callbacks[event]) do
            if type(callback) == 'function' then callback(ev, data) end
        end
    end
end
...
function sv_events:silence(event)
    if not self._Silenced[event] then self._Silenced[event] = true end

    return self._Silenced[event]
end

--- Remove the silencing of an event.
function sv_events:unsilence(event) self._Silenced[event] = nil end

--- Checks to see if an event is silenced.
function sv_events:isSilenced(event) return self._Silenced[event] end

RegisterModule('sv_events', sv_events)
return sv_events
